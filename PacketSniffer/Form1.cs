...
namespace PacketSniffer
{
    ...
    public partial class Form1 : Form
    {
        ...
        private void tsbtnStart_Click(object sender, EventArgs e)
        {
            if(this.tscbIp.Text=="")
            {
                MessageBox.Show("캡처할 네트워크 인터페이스를 선택하세요.", "알림", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
        ...
        private void OnReceive(IAsyncResult result)
        {
            try
            {
                int nReceived = mainSocket.EndReceive(result);
        ...
        {
            string[] IPNode = new string[] 
            {
                "IP", "Ver: " + ipHeader.Version,
        ...
        private void ReceivedPacketListView_Click(object sender, EventArgs e)
        {
            this.PacketDetailTreeView.Nodes.Clear();
            PacketDetailTreeView.Nodes.Add(this.ReceivedPacketListView.SelectedItems[0].SubItems[6].Text);

            for (int n = 7; n < 19; n++)
                PacketDetailTreeView.Nodes[0].Nodes.Add(this.ReceivedPacketListView.SelectedItems[0].SubItems[n].Text);

            if(this.ReceivedPacketListView.SelectedItems[0].SubItems[4].Text=="TCP")
            {
                PacketDetailTreeView.Nodes.Add(this.ReceivedPacketListView.SelectedItems[0].SubItems[19].Text);
                for (int n = 20; n < 28; n++)
                    PacketDetailTreeView.Nodes[1].Nodes.Add(this.ReceivedPacketListView.SelectedItems[0].SubItems[n].Text);
            }
            else if(this.ReceivedPacketListView.SelectedItems[0].SubItems[4].Text=="UDP")
            {
                PacketDetailTreeView.Nodes.Add(this.ReceivedPacketListView.SelectedItems[0].SubItems[19].Text);
                for (int n = 20; n < 24; n++)
                    PacketDetailTreeView.Nodes[1].Nodes.Add(this.ReceivedPacketListView.SelectedItems[0].SubItems[n].Text);
            ...
            this.PacketDetailTreeView.ExpandAll();
        }
    }
}
